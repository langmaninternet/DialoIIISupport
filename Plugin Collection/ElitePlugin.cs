...
namespace Turbo.Plugins.Default
{
    public class ElitePlugin : BasePlugin, IInGameWorldPainter
    {
        ...
        /* StrickenPlugin                                                       */
        /************************************************************************/
        public int StrickenGemLevel { get; set; }
        ...
        public void DrawStrickenPlugin(WorldLayer layer)
        {
            ...
            foreach (var StrickenLocation in jewelsLocations)
            {
                ...
                if (Stricken.SnoItem.Sno == 3249948847) { StrickenActive = true; StrickenGemLevel = Stricken.JewelRank; break; } else { continue; }
            }
            if (StrickenActive == false) return;
            float gemMaths = 0.8f + (0.01f * (float)StrickenGemLevel);
